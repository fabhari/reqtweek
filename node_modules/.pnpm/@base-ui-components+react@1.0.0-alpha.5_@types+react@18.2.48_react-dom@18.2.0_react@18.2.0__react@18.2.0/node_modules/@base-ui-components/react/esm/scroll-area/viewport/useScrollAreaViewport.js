...
export function useScrollAreaViewport(params) {
  const {
    children
  } = params;
  const {
  ...
  const computeThumb = useEventCallback(() => {
    ...
    const clampedNextHeight = Math.max(MIN_THUMB_SIZE, nextHeight - scrollbarYOffset - thumbYOffset);
  ...
  const getViewportProps = React.useCallback((externalProps = {}) => mergeReactProps(externalProps, {
    ...(rootId && {
  ...
  }), [rootId, hiddenState.scrollbarXHidden, hiddenState.scrollbarYHidden, children, computeThumb, handleScroll, viewportRef]);
